     ...
     async chat(c) {
        try {
            ...
            if (!sessionId || !message || typeof message !== "string" || message.trim() === "") {
                return c.json(
                    {
                        error: "Os campos 'sessionId' e 'message' são obrigatórios.",
                        example: {
                            sessionId: "user-1728080000000-abcdef123",
                            message: "Como estará o tempo em Paris em maio de 2026?",
                        },
                    },
                    400,
                );
            ...
            const response = await agentService.chat(message.trim(), sessionId);
